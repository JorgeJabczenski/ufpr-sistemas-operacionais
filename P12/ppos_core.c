...
int mqueue_create(mqueue_t *queue, int max, int size)
{
    ...
    sem_create(&(queue->sem_vaga), max);
    sem_create(&(queue->sem_buff), 1);
...
int mqueue_send(mqueue_t *queue, void *msg)
{
    ...
    if (sem_down(&(queue->sem_vaga)) != 0)
        return -1;
    if (sem_down(&(queue->sem_buff)) != 0)
        return -1;

    queue->quantidadeMensagens++;
    memcpy(queue->buffer + (queue->buffCabeca * queue->tamanhoMensagem), msg, queue->tamanhoMensagem);
    queue->buffCabeca = (queue->buffCabeca + 1) % queue->tamanhoMaximo;
...
int mqueue_recv(mqueue_t *queue, void *msg)
{
    ...
    if (sem_down(&(queue->sem_item)) != 0)
        return -1;
    if (sem_down(&(queue->sem_buff)) != 0)
        return -1;

    queue->quantidadeMensagens--;
    memcpy(msg, queue->buffer + (queue->buffRabo * queue->tamanhoMensagem), queue->tamanhoMensagem);
    queue->buffRabo = (queue->buffRabo + 1) % queue->tamanhoMaximo;
...
{
    return queue->quantidadeMensagens;
}